async function hashSHA(buffer, algorithm) {
...
function hashMD5(buffer) {
  ...
  function md5blks(s) {
    ...
    for (let i = 0; i < len; i++) blks[i >> 2] |= s[i] << ((i % 4) * 8);
    blks[len >> 2] |= 0x80 << ((len % 4) * 8);
    blks[nblk * 16 - 2] = len * 8;
    return blks;
  }

  const x = md5blks(bytes);
  ...
  return bufToHex(new Uint32Array([a, b, c, d]).buffer, true);
}

function bufToHex(buffer, littleEndian = false) {
  ...
  if (littleEndian) {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      hex += word.toString(16).padStart(8, '0').match(/../g).reverse().join('');
    }
...
async function computeHash(algorithm, buffer) {
  switch (algorithm) {
    case 'MD5':      return hashMD5(buffer);
    case 'SHA-1':    return hashSHA(buffer, 'SHA-1');
    case 'SHA-256':  return hashSHA(buffer, 'SHA-256');
    case 'SHA-384':  return hashSHA(buffer, 'SHA-384');
    case 'SHA-512':  return hashSHA(buffer, 'SHA-512');
